...
### GH013: Secret Scanning Blocks Push (Even for Historical Commits)

GitHub Secret Scanning runs on every push and **scans the entire reachable commit graph**, not just HEAD. If any commit in the chain (even the very first one) contains a detected secret pattern, the entire push is rejected with:
...
remote:        locations:
remote:          - commit: 6483fd3e3d7e04b7e6415d7e6c1f7a3f609fc4ab
remote:            path: .env:99
```

**Why `git commit --amend` doesn't fix it:** Amending rewrites only the current HEAD commit. If the commit with the secret is an ancestor of HEAD (e.g. the first commit), it's still reachable in the object graph, and Secret Scanning will still find it.

**The correct fix — reinitialize repo from scratch:**
```bash
# Step 1: Wipe the local git history entirely
rm -rf .git

# Step 2: Re-initialize fresh
git init
git config user.email "hermes@backup.local"
git config user.name "Hermes Backup"

# Step 3: Set remote (git init removes it)
TOKEN=$(sed -n 's/^GITHUB_TOKEN=//p' /home/eddellar/.hermes/.env | tr -d '\r\n')
git remote add origin "https://${TOKEN}@github.com/owner/repo.git"

# Step 4: Copy files with secrets already REDACTED at copy time (see below)
...
# Step 5: Remove nested .git dirs BEFORE git add
find . -name '.git' -type d -exec rm -rf {} + 2>/dev/null || true

# Step 6: git add -A — now makes a single clean commit with no secret history
git add -A
...
**Why this works:** Starting from scratch means there is no commit history containing secrets. The entire reachable object graph is the single new commit, which has no secrets.

**Prevention — redact secrets at copy time, not after:**
```bash
# Redact ALL variable assignments at copy time using sed
cat /home/eddellar/.hermes/.env \
    | sed -e 's/^GITHUB_TOKEN=.*/GITHUB_TOKEN=REDACTED/' \
          -e 's/^HF_TOKEN=.*/HF_TOKEN=REDACTED/' \
          -e 's/^FEISHU_APP_SECRET=.*/FEISHU_APP_SECRET=REDACTED/' \
          -e 's/^API_SERVER_KEY=.*/API_SERVER_KEY=REDACTED/' \
          -e 's/^TERMINAL_MODAL_IMAGE=.*/TERMINAL_MODAL_IMAGE=REDACTED/' \
    > .env
```

**Common secret patterns GitHub detects automatically:**
- `GITHUB_TOKEN=ghp_...` — GitHub Personal Access Token
- `HF_TOKEN=hf_...` — HuggingFace User Access Token
- `KIMI_API_KEY=`, `DEEPSEEK_API_KEY=`, `MINIMAX_API_KEY=` — other provider keys
- Any `=gh_`, `=hf_`, `=sk_` prefix patterns

**Do NOT use git-filter-repo for this:** While `git filter-repo --invert-paths --force` can remove specific files from history, it does NOT strip secret values from file content — it only removes the file path. The secrets in `.env` are still in the commit objects themselves.
...
### ⚠️ Critical: Never Commit Real Secrets — Redact at Copy Time

GitHub Secret Scanning runs on every push and **will block the entire push** if a commit contains a detected secret token, even in an ancestor commit. `git commit --amend` does NOT fix this — it only rewrites HEAD.

**The complete workflow for .env-type files:**

```bash
# Step 1: Copy .env but REDACT all values at copy time (NOT after)
python3 -c "
import sys
...
**Alternative — sed-based redaction (avoids Python):**
```bash
cat /home/eddellar/.hermes/.env \
    | sed -e 's/^GITHUB_TOKEN=.*/GITHUB_TOKEN=[REDACTED]/' \
          -e 's/^HF_TOKEN=.*/HF_TOKEN=[REDACTED]/' \
...
# 3. Copy important items (config.yaml, skills, cron — skip .env here, handle separately above)
for item in config.yaml skills cron; do
    [ -e "/home/eddellar/.hermes/$item" ] && cp -r "/home/eddellar/.hermes/$item" ./
done

# 3b. Copy and REDACT .env (do this as the LAST item so the redirect works cleanly)
python3 -c "
import sys
with open('/home/eddellar/.hermes/.env') as f:
    for line in f:
...
# 4. Remove nested .git dirs BEFORE git add (skills may contain embedded repos)
# Step 4a: Un-track embedded repos from git index FIRST
git rm --cached -rf skills/.archive/comfyui-skill-openclaw \
            skills/creative/comfyui-workflow \
            skills/creative/js-designer-skill 2>/dev/null || true
# Step 4b: Then remove .git dirs from filesystem
find . -name '.git' -type d -exec rm -rf {} + 2>/dev/null || true
...
# 6. Push — use --set-upstream on first push; if rejected due to pre-existing remote history,
#    fall back to a named branch
git push --set-upstream origin master 2>/dev/null || \
  git push origin master:backup_$(date +%Y%m%d)

# 7. Clean up — cd out FIRST to avoid getcwd panic, then remove
cd / && rm -rf /tmp/hermes_git_backup
```

> **Why `cd /` before `rm -rf`**: Removing the current working directory while the shell's CWD is inside that directory causes a `getcwd: cannot access parent directories` error on the next command. Always `cd /` before deleting.

> **Why push to a named branch as fallback**: When the remote has pre-existing history (e.g. a previous backup push that succeeded partially), `git push --set-upstream` fails with "remote contains work you do not have". Pushing to `backup_$(date +%Y%m%d)` creates an independent snapshot without requiring force or rebase.
...
# 5. Clean nested .git dirs (skills may contain embedded repos) — CRITICAL ORDER below
# Step 5a: Un-track embedded repos from git index FIRST
git rm --cached -rf skills/.archive/comfyui-skill-openclaw \
            skills/creative/comfyui-workflow \
            skills/creative/js-designer-skill 2>/dev/null || true
# Step 5b: Then remove .git dirs from filesystem
find . -name '.git' -type d -exec rm -rf {} + 2>/dev/null || true
...
When copying a directory tree (e.g. a skills directory that may contain embedded git repos as subdirectories), `git add` will recursively include all nested `.git` dirs as if they were part of the parent. **Correct order — `git rm --cached` FIRST, then filesystem removal:**

```bash
# Step 1: Tell git to stop tracking the embedded repos (prevents gitlink/submodule references)
git rm --cached -rf skills/.archive/comfyui-skill-openclaw \
            skills/creative/comfyui-workflow \
            skills/creative/js-designer-skill 2>/dev/null

# Step 2: Remove the actual .git directories from the filesystem
find . -name '.git' -type d -exec rm -rf {} + 2>/dev/null || true

# Step 3: Re-add everything cleanly
git add -A
```

> **Why `git rm --cached` first:** Without it, git follows the `.git` directory as a nested submodule reference when you later run `find . -name '.git' -exec rm -rf {}`. The `git rm --cached` breaks the index reference first, making the subsequent filesystem deletion safe.
...
| `remote: error: GH013: Repository rule violations` | Secret Scanning detected secrets in commit history. `git commit --amend` won't fix it — the secret is in an ancestor commit. Reinitialize the repo from scratch: `rm -rf .git; git init; ...` with secrets redacted at copy time. See GH013 section above. |
| `git push` times out / hangs | GitHub LFS tracking or large pack upload can stall. Try: `git config http.version HTTP/1.1 && git config http.postBuffer 524288000 && git push`. If that fails, push to a new branch: `git push origin master:backup_$(date +%Y%m%d)` |
