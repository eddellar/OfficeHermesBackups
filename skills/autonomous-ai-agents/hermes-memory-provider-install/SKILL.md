...
## TencentDB Agent Memory — Installation Reference

> **Status:** ✅ Successfully installed 2026-05-16.
> Package: `@tencentdb-agent-memory/memory-tencentdb@0.3.5` (MIT license)

### Verified Working Installation (2026-05-16)

**Verified working on:** WSL2 + Hermes v0.12.0 + Node.js v23.11.1 + npm 11.13.0.

### Architecture

```
Hermes Agent (Python)
  └─ MemoryProvider adapter (Python, thin HTTP client)
       └─ POST /recall, /capture, /search → 127.0.0.1:8420
            memory-tencentdb Gateway (Node.js)
               └─ L0-L3 memory management + SQLite + sqlite-vec
```

This means:
- Node.js runtime required (`node >= 22.16`)
- Gateway auto-discovers `src/gateway/server.ts` from well-known paths
- LLM API key required for L1/L2/L3 extraction (passed via env vars to Gateway)
- Data dir: `~/.memory-tencentdb/memory-tdai/` (Gateway-owned)
- `pnpm` NOT required — uses `npx tsx` (Node.js ESM executor)

### Installation Steps (Verified Working)

**1. Download tarball (avoids npm registry search timeout):**
```bash
...
**2. Install Node.js dependencies (use --omit=dev, foreground with timeout):**
```bash
cd ~/.memory-tencentdb/tdai-memory-openclaw-plugin
npm install --omit=dev  # ~19s on fast connection
# NOTE: dev dep @vitest/utils@4.1.6 doesn't exist in registry — omit=dev sidesteps this
# Also install tsx separately if not present (it's a dev dep):
npm install tsx --omit=dev
```

**3. Symlink plugin into Hermes:**
```bash
...
# Or for auto-start (GatewaySupervisor handles this):
# npx tsx src/gateway/server.ts
```
...
# {"status":"ok","version":"0.1.0","uptime":10,"stores":{"vectorStore":true,"embeddingService":false}}
```

### Gateway API Endpoints (verified)
...
**`embeddingService: false` after startup:**
- This is expected without LLM API key configured
- L0 capture still works, but L1/L2/L3 extraction requires:
  ```bash
...
**`tsx` not found:** Install it explicitly — it's a dev dependency:
  ```bash
  npm install tsx --omit=dev
  ```
...
| What | Path |
|------|------|
| Gateway entry | `~/.memory-tencentdb/tdai-memory-openclaw-plugin/src/gateway/server.ts` |
| Hermes Provider | `~/.memory-tencentdb/tdai-memory-openclaw-plugin/hermes-plugin/memory/memory_tencentdb/` |
| Data (L0-L3) | `~/.memory-tencentdb/memory-tdai/` |
| SQLite DB | `~/.memory-tencentdb/memory-tdai/vectors.db` |
| Tables | `l0_conversations`, `l1_records`, `embedding_meta` |

### Gateway Auto-Discovery
If `MEMORY_TENCENTDB_GATEWAY_CMD` is unset, the Python Provider searches for `server.ts` in order:
1. In-tree (when Hermes loads from plugin repo checkout)
2. `~/.memory-tencentdb/tdai-memory-openclaw-plugin/src/gateway/server.ts` (preferred, verified found)
...
### Four-Layer Memory Architecture (L0→L3)
| Layer | Content | Storage |
|-------|---------|---------|
| L0 | Raw conversation | SQLite `l0_conversations` table + JSONL |
| L1 | Atomic facts (LLM-extracted) | SQLite `l1_records` + sqlite-vec |
| L2 | Scene blocks | Markdown files under `scene_blocks/` |
| L3 | User persona | `persona.md` |

**Short-term compression:** Tool results offloaded to `refs/*.md`, context holds only Mermaid task graph with `node_id` references. Original evidence traceable via `node_id → refs/*.md`.
