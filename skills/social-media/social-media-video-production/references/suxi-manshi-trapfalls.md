...
**绕过方案：** 从工作流中**移除所有音频相关节点**，只保留纯视频 latent 链：
```
...
## 3. Z-Image-Turbo NunchakuZImageDiTLoader 崩溃（2026-05-13）

**症状：** `NunchakuZImageDiTLoader` 加载 `z_image_turbo_bf16.safetensors` 时崩溃，报 `KeyError: 'weight'`。错误发生在 `unchaku/utils.py` 的 `get_precision_from_quantization_config` 函数。

**影响：** 直接 REST API 调用 Z-Image-Turbo 全部失败。

**绕过方案：** 图像生成改用 SD1.5 + MajicmixRealistic：
```
CheckpointLoaderSimple(SD1.5\majicmixRealistic_v7.safetensors)
→ CLIPTextEncode → CLIPTextEncode(negative)
→ EmptyLatentImage → KSampler(steps=25, cfg=7.0)
→ VAEDecode → SaveImage
```

**状态：** `comfyui-skill` CLI 内部可绕过此问题（其内部处理方式不同）；直接 REST API 调用必须用 SD1.5 替代。

---

## 视频生成推荐路径（2026-05-13 实测）

| 方案 | 状态 | 调用方式 |
|------|------|---------|
| **LTX 2.3（纯视频）** | ⭐ 推荐 | `comfyui-skill generate --workflow ltx_23_t2v_distill` |
| 图像（Z-Image-Turbo） | ⚠️ 降级 | 直接 API 用 SD1.5 + Majicmix；CLI 可用 Z-Image-Turbo |
